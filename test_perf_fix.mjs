...
async function checkBloat() {
   ...
   const mbSize = (Buffer.byteLength(JSON.stringify(data), 'utf8') / 1024 / 1024).toFixed(2);
   console.log(`[LEGACY select(*)] Fetched ${data.length} records in ${time}ms.`);
   console.log(`[LEGACY select(*)] Payload size: ${mbSize} MB`);

   // Projection
   const start2 = Date.now();
   
   // DTO Field Set (Everything strictly necessary for FigureCard + minimal filter fallback mappings as currently mapped)
   const SELECT_FIELDS = `
     ...
     series:series_id(id, series_name, title, slug_tr, slug, slug_en, series_number, number, category_main, category, manual_rarity, rarity, final_rarity)
   `;
