        ...
        <div className="lg:col-span-8 pb-20">
          <form onSubmit={handleSave} className="space-y-0 w-full text-[13px] font-bold">
            ...
            <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden mb-8">
                ...
                {/* DEĞER */}
                <div className="flex border-b border-gray-100 items-center hover:bg-gray-50 transition-colors group">
                    <div className="w-1/3 py-4 pr-4 pl-6 border-l-2 border-transparent group-hover:border-black transition-colors"><label className="text-gray-900 block font-black">Değer (USD)</label></div>
                ...
                {/* NADİRLİK DERECESİ (Dinamik Dropdown) */}
                <div className="flex border-b border-gray-100 items-center hover:bg-gray-50 transition-colors group">
                    <div className="w-1/3 py-4 pr-4 pl-6 border-l-2 border-transparent group-hover:border-black transition-colors"><label className="text-gray-900 block font-black">Nadirlik Derecesi</label></div>
                    <div className="w-2/3 py-2">
                        <select value={customAttributes['nadirlik-derecesi'] || ''} onChange={(e) => handleCustomAttrChange('nadirlik-derecesi', e.target.value)} className="w-full bg-transparent px-3 py-2 focus:outline-none text-black font-bold appearance-none cursor-pointer">
                            <option value="">Seçim Yapılmadı</option>
                            {categories.filter(c => c.type === 'nadirlik-derecesi').map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                        </select>
                    </div>
                </div>
