...
export default function SettingsForm({ initialData }: { initialData: any }) {
  ...
  const handleProfileUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (res?.error) {
          ...
          setProfileMsg({ text: 'Profil bilgilerin başarıyla güncellendi!', type: 'success' });
          router.refresh();
      }
  };
  ...
  const handlePasswordUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (res?.error) {
          ...
          setPasswordMsg({ text: 'Şifren güvenli bir şekilde güncellendi!', type: 'success' });
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
      ...
      <section className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
          <h2 className="text-2xl font-black text-gray-900 mb-6 border-b border-gray-100 pb-4">Profil Bilgileri</h2>
          ...
          <form onSubmit={handleProfileUpdate} className="space-y-5">
              <div className="space-y-1">
                  <label className="text-[11px] font-black uppercase tracking-widest text-gray-500">Kayıtlı E-Posta Adresi</label>
                  <input 
                      type="email" 
                      name="email"
                      ...
                      required
                  />
                  <p className="text-[10px] text-gray-400 font-semibold mt-1">Değiştirirseniz güvenlik onayı almanız gerekecektir.</p>
              </div>

              <div className="space-y-1">
                  <label className="text-[11px] font-black uppercase tracking-widest text-gray-500">Adı Soyadı</label>
                  <input 
                      type="text" 
                      name="full_name"
                      defaultValue={initialData?.full_name || ''} 
                      className="w-full bg-gray-50 border border-gray-200 text-gray-900 font-bold px-4 py-3 rounded-xl focus:ring-2 focus:ring-[#D22B2B] focus:border-transparent outline-none transition-all" 
                      placeholder="Adını gir (İsteğe Bağlı)" 
                  />
              </div>

              <div className="space-y-1">
                  <label className="text-[11px] font-black uppercase tracking-widest text-gray-500">Yaş</label>
                  <input 
                      type="number" 
                      name="age"
      ...
      {/* ŞIFRE FORMU */}
      <section className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
          <h2 className="text-2xl font-black text-gray-900 mb-6 border-b border-gray-100 pb-4">Güvenlik ve Şifre</h2>
          ...
          <form onSubmit={handlePasswordUpdate} className="space-y-5">
              <div className="space-y-1">
                  <label className="text-[11px] font-black uppercase tracking-widest text-gray-500">Yeni Şifre</label>
                  <input 
                      type="password" 
                      name="new_password"
                      ...
                      placeholder="Gizli şifreni gir (En az 6 karakter)" 
                      required
                  />
              </div>

              <div className="space-y-1">
                  <label className="text-[11px] font-black uppercase tracking-widest text-gray-500">Yeni Şifre (Tekrar)</label>
                  <input 
                      type="password" 
