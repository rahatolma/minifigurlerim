...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const data = legalContent[locale]['hak-ihlali'];
  
  const trPath = `/tr/yasal/hak-ihlali`;
  const enPath = `/en/legal/rights-violation`;
...
export default async function DMCA({ params }: { params: Promise<{ locale: string }> }) {
  const resolvedParams = await params;
  const locale = resolvedParams.locale || 'tr';
  const data = legalContent[locale]['hak-ihlali'];
