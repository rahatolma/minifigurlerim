...
export async function GET() {
  ...
  figures.forEach((f: any) => {
    if (!f.slug_en || !f.series.slug_en) return;

    const url = `${baseUrl}/en/figures/${f.series.slug_en}/${f.slug_en}`;
    ...
    const altTr = (f.slug_tr && f.series.slug_tr) 
      ? `\n      <xhtml:link rel="alternate" hreflang="tr-TR" href="${baseUrl}/tr/figurler/${f.series.slug_tr}/${f.slug_tr}" />`
      : '';

    const xDefault = (f.slug_tr && f.series.slug_tr)
      ? `\n      <xhtml:link rel="alternate" hreflang="x-default" href="${baseUrl}/tr/figurler/${f.series.slug_tr}/${f.slug_tr}" />`
