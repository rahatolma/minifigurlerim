    ...
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-gray-200 pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between px-2 h-[70px]">
          ...
          <Link href="/" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
            ...
            <span className={`text-[10px] font-bold tracking-wide ${isActive('/') ? 'text-[#D22B2B]' : 'text-gray-400'}`}>Ana Sayfa</span>
          </Link>
          ...
          <Link href="/seriler" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
            ...
            <span className={`text-[10px] font-bold tracking-wide ${isActive('/seriler') ? 'text-[#D22B2B]' : 'text-gray-400'}`}>Seriler</span>
          </Link>
          ...
          <div className="flex flex-col items-center justify-center w-full h-full relative">
            ...
            <span className={`text-[10px] font-bold tracking-wide absolute bottom-2 ${isActive('/koleksiyonum') ? 'text-black' : 'text-gray-600'}`}>Koleksiyon</span>
          </div>
          ...
          <Link href="/figurler" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
            ...
            <span className={`text-[10px] font-bold tracking-wide ${isActive('/figurler') ? 'text-[#D22B2B]' : 'text-gray-400'}`}>Figürler</span>
          </Link>
          ...
          <button onClick={() => setDrawerOpen(true)} className="flex flex-col items-center justify-center w-full h-full gap-1 group">
            ...
            <span className="text-[10px] font-bold tracking-wide text-gray-400">Menü</span>
          </button>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-black text-gray-900 tracking-tighter">İşlemler & Menü</h2>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-100 flex items-center justify-between">
           <div>
             <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Hesap</p>
             <p className="text-sm font-black text-gray-900">{user?.email || 'Misafir Kullanıcı'}</p>
           </div>
           {!user ? (
             <Link href="/login" onClick={() => setDrawerOpen(false)} className="bg-black text-white text-xs font-bold px-4 py-2 rounded-lg">Giriş Yap</Link>
           ) : (
             <Link href="/koleksiyonum/ayarlar" onClick={() => setDrawerOpen(false)} className="bg-white border border-gray-200 text-gray-900 shadow-sm text-xs font-bold px-4 py-2 rounded-lg">Ayarlar</Link>
           )}
        </div>

        <nav className="flex flex-col gap-2">
          <Link href="/lego-hakkinda" onClick={() => setDrawerOpen(false)} className="px-4 py-3 bg-white rounded-xl font-bold text-gray-700 active:bg-gray-50 border border-transparent active:border-gray-100">LEGO® Hakkında</Link>
          <Link href="/hakkimizda" onClick={() => setDrawerOpen(false)} className="px-4 py-3 bg-white rounded-xl font-bold text-gray-700 active:bg-gray-50 border border-transparent active:border-gray-100">Hakkımızda</Link>
          <Link href="/haberler" onClick={() => setDrawerOpen(false)} className="px-4 py-3 bg-white rounded-xl font-bold text-gray-700 active:bg-gray-50 border border-transparent active:border-gray-100">Blog / Haberler</Link>
          <Link href="/iletisim" onClick={() => setDrawerOpen(false)} className="px-4 py-3 bg-white rounded-xl font-bold text-gray-700 active:bg-gray-50 border border-transparent active:border-gray-100">İletişim</Link>
          ...
          <button onClick={() => {
            ...
            Telif Hakkı & Yasal Uyarı <span className="text-gray-400 text-xs">Aç</span>
          </button>
        </nav>
      </div>
    </>
